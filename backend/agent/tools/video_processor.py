...
async def youtube_to_docx(
    ...
    expire_seconds: int = 86400  # Document expires after 1 day by default
) -> str:
    """
    Загружает YouTube видео, извлекает изображения каждые 5 секунд,
    удаляет схожие изображения и сохраняет DOCX-документ в Redis с уникальным ключом.

    :param youtube_url: Ссылка на YouTube видео.
    :param redis_service: Instance of RedisService to interact with Redis.
    :param expire_seconds: Time in seconds after which the document expires in Redis.
    :return: Уникальный ключ, по которому можно получить DOCX файл из Redis.
    """
    try:
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            # Создание DOCX документа в памяти
            doc = Document()
            ...
            # Генерация уникального ключа для Redis
            unique_key = f"docx:{uuid.uuid4()}"

            # Сохранение DOCX в Redis
            success = await redis_service.set_file(unique_key, doc_bytes, expire=expire_seconds)
            if not success:
                logger.error("Не удалось сохранить DOCX файл в Redis.")
                raise HTTPException(status_code=500, detail="Не удалось сохранить документ в хранилище.")

            logger.info(f"DOCX документ сохранен в Redis с ключом: {unique_key}")
            return unique_key
